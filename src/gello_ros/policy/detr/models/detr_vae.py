...
class DETRVAE_Comp(nn.Module):
    """This is the DETR module that performs object detection"""

    def __init__(
        self, backbones, transformer, encoder, state_dim, num_queries, camera_names, include_ft=False, ft_dim=6
    ):
        """Initializes the model.
        Parameters:
            backbones: torch module of the backbone to be used. See backbone.py
        ...
        if self.include_ft:
            ...
            self.decoder_ft_proj = nn.Linear(ft_dim, hidden_dim)  # project ft to embedding
            self.additional_pos_embed = nn.Embedding(
                3, hidden_dim
            )
        else:
    ...
    def forward(self, qpos, image, ft, env_state, actions=None, is_pad=None):
        """
        qpos: batch, qpos_dim
        image: batch, num_cam, channel, height, width
        env_state: None
        ...
        if is_training:
            ...
            if self.include_ft:
                ft_embed = self.encoder_ft_proj(ft)
